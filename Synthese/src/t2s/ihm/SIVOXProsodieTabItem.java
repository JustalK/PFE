...
public class SIVOXProsodieTabItem extends CTabItem {
	...
	public void loadXML(Element prosodie) throws SIVOXException
	{
		try {
			...
			List<Element> liste = (List<Element>)elementListePhoneme.getChildren("Phoneme");
			Iterator<Element> j = liste.iterator();
			while(j.hasNext())
			{
				Element elementPhoneme = j.next();
				TableItem itemLigne = new TableItem(tableProsodie, SWT.NONE);
				String [] ligne = new String[16];
				ligne[0] = elementPhoneme.getChild("Data").getText();
				ligne[1] = elementPhoneme.getChild("Longueur").getText();
				@SuppressWarnings("unchecked")
				List<Element> listeCouple = (List<Element>)elementPhoneme.getChild("ListeCouple").getChildren("Couple");
				Iterator<Element> k = listeCouple.iterator();
				int indice = 2;
				while((k.hasNext()) && (indice <= 15))
				{
					Element elementCouple = k.next();
	...
	public void jouerProsodie(int [] tabSelection) throws SIVOXException
	{
		try {
			...
			synthetiseur = new SynthetiseurMbrola(jk, lt.getVoix(),ConfigFile.rechercher("REPERTOIRE_PHO_WAV")+"/",ConfigFile.rechercher("FICHIER_PHO_WAV"));
			synthetiseur.play(false);
		} catch (SIVOXException e) {
			throw e;
		} catch (Exception z) {
	...
	public void sauverWav(int [] tabSelection) throws SIVOXException
	{
		try {
			lt.setVoix(i.getVoix());
			enregistrerMorceauProsodie(tabSelection, ConfigFile.rechercher("REPERTOIRE_PHO_WAV")+"/"+ConfigFile.rechercher("FICHIER_PHO_WAV")+".pho");
			SynthetiseurMbrola synthetiseur = new SynthetiseurMbrola(jk, lt.getVoix(),ConfigFile.rechercher("REPERTOIRE_PHO_WAV")+"/",ConfigFile.rechercher("FICHIER_PHO_WAV"));
			synthetiseur.muet();
