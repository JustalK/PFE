...
public class Chant {
	...
	private JukeBox jk = new JukeBox();
	private LecteurTexte lt = new LecteurTexte(jk);
	...
	private void chargerSVC(String nomFichierSVC) throws SIVOXException
	{
		...
		try {
			...
			List<Element> listeSyllabe = (List<Element>)elementTexte.getChildren("Syllabe");
			Iterator<Element> j = listeSyllabe.iterator();
			while(j.hasNext())
			{
				Element elementSyllabe = j.next();
				texte.add(elementSyllabe.getText());
			}
			Element elementMelodie = elementRacine.getChild("Melodie");
			List<Element> listeNote = (List<Element>)elementMelodie.getChildren("Note");
			Iterator<Element> k = listeNote.iterator();
			while(k.hasNext())
			{
				Element elementNote = k.next();
	...
	public String toStringPho() throws ChantException , NoteException
	{
		if(isValide())
		{
			LecteurTexte lt = new LecteurTexte(jk);
	...
	public void chanter() throws ChantException , NoteException
	{
		try {
			...
			sb = new SynthetiseurMbrola(jk, voix,ConfigFile.rechercher("REPERTOIRE_PHO_WAV") + ConfigFile.rechercher("FICHIER_PHO_CHANT"), "");
			sb.play(false);
