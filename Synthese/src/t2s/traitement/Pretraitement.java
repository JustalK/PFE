...
public class Pretraitement {
	...
	private static final String[]	nombre	= { "", "un", "deux", "trois", "quatre", "cinq", "sisse", "sept", "huit", "neuf", "dix", "onze", "douze", "treize", "quatorze", "quinze", "seize",
	        "dix_sept", "dix_huit", "dix_neuf" };
	private static final String[]	dizaine	= { "", "", "vingt", "trente", "quarante", "cinquante", "soixante", "", "quatre_vingt", "" };
	 ...
	 * <p>
	 * <b>Remarque</b> : Si la chaine <code>txt</code> ne finit pas par un caractere de coupure (<i>ponctuation</i>), on rajoute un <code>'.'</code> de chaine.
	 * </p>
	 * 
	 * @param txt
	 *            la chaine de caracteres a pre - traiter
	 */
	public Pretraitement(final String txt) {
		this.texte = txt;
		if (!"".equals(this.texte) && !estCoupure(this.texte.charAt(this.texte.length() - 1))) {
			this.texte += ".";
		}
	}
	 ...
	 * <p>
	 * <b>Remarque</b> : Par effet de bord, on avance (sans espoir de retour) dans le texte pre-traite.
	 * </p>
	 * 
	 ...
	 * <p>
	 * <b>Pre-traitement</b> : il faut rajouter le traitement des symboles bizarres...
	 * <p>
	 * <ul>
	 * j'aime ubuntu
	...
	private Phrase traiter() {
		...
		char tete = this.texte.charAt(i);
		while ((i < this.texte.length()) && !estCoupure(tete)) {
			if (Character.isUpperCase(tete)) {
				final Indice in = new Indice(i);
				final String a = abbrev(this.texte, in);
				if (a.length() > 0) {
					buffer.append(a);
					i = in.val();
				} else {
				...
				if (tete == '+') {
					buffer.append("_plusse_");
				} else if ((tete == '/') && estNum) {
					buffer.append("_sur_");
				} else if ((tete == '*') && estNum) {
					buffer.append("_fois_");
				} else if ((tete == '-') && estNum) {
					buffer.append("_moins_");
				} else if ((tete == '=') && estNum) {
					buffer.append("_�gal_");
				}
				i++;
			} else if (estChiffre(tete)) {
				final Indice ind = new Indice(i);
				estNum = true;
				buffer.append(traiterLesChiffres(this.texte, ind));
				i = ind.val();
				if (i < this.texte.length()) {
					tete = this.texte.charAt(i);
				}
		...
		{
			while ((i < this.texte.length()) && estCoupure(this.texte.charAt(i))) {
				i++;
			}
			if (buffer.length() != 0) {
				...
				if (i < this.texte.length()) {
					this.texte = this.texte.substring(i);
				} else {
					this.texte = "";
				}
	 ...
	 */
	private String traiterLesChiffres(final String texte, final Indice ind) {
		final int lg = texte.length();
		String nbr = lireNombre(texte, ind);
		StringBuffer buffer = traiterNombre(nbr, false);
		final String ordinal = ordinal(texte, ind);
		if (ordinal.length() > 0) {
			...
			if (ind.val() < lg) {
				final char tete = texte.charAt(ind.val());
				if (estVirgule(tete) || estHeure(tete)) {
					final int iSuiv = ind.val() + 1;
	 ...
	 * texte[i.val()] n'est pas un chiffre
	 */
	private String lireNombre(final String texte, final Indice i) {
		final StringBuffer buffer = new StringBuffer();
		final int lg = texte.length();
	 ...
	 * <p>
	 * <center><code>12 |--> douze</code></center>
	 * </p>
	 * <p>
	 * <b>Remarque</b>: On ne gere pas le feminin (et puis quoi encore ??)
	 * </p>
	 * 
	 * @param nbr
	 *            une chaine de caractere representant un nombre
	 * @param decimal
	 *            un booleen pour savoir si l'on doit prononcer les '0'.
	 * @return une chaine de caractere contenant le nombre sous sa forme textuelle.
	 */
	private StringBuffer traiterNombre(String nbr, final boolean decimal) {
		final StringBuffer buffer = new StringBuffer();
		if (decimal) {
		...
		try {
			...
			if (nombre == 0) {
				return new StringBuffer("z�ro");
			}
			final int milliard = nombre / MILLIARD;
			nombre = nombre % MILLIARD;
			final int million = nombre / MILLION;
			nombre = nombre % MILLION;
			final int mille = nombre / MILLE;
	 ...
	 * 
	 * @param nbr
	 *            la chaine de caractere representant le nombre a ne pas traiter
	 */
	
	private String pasTraiterNombre(final String nbr) {
		final StringBuffer buffer = new StringBuffer();
		int i = 0;
		int n;
		while (i < nbr.length()) {
	 ...
	 * 
	 * @param c
	 *            le caractere a tester.
	 * @return true si c est un c est un caractere de coupure {, . ; : ! ? parentheses}
	 */
	private boolean estCoupure(final char c) {
		return (c == ',') || (c == '.') || (c == ';') || (c == ':') || (c == '!') || (c == '?') || estParenthese(c) || (c == '|');
	}
	
	@SuppressWarnings("unused")
	private boolean estPonctuation(final char c) {
		return (c == '.') || (c == '?') || (c == ';') || (c == '!') || (c == ',') || (c == '/');
	}
	
	@SuppressWarnings("unused")
	private boolean estFindePhrase(final char c) {
		return (c == '.') || (c == '?') || (c == ';') || (c == '!');
	}
	 ...
	 */
	private boolean estSpecial(final char c) {
	 ...
	 */
	private boolean estParenthese(final char c) {
	 ...
	 * 
	 * @param c
	 *            le charactere a tester
	 * @return true si c est un caractere de 'virgule' {, .}
	 */
	private boolean estVirgule(final char c) {
		return (c == ',') || (c == '.');
	}
	
	private boolean estOperateur(final char c) {
		return (c == '+') || (c == '-') || (c == '/') || (c == '*') || (c == '=');
	}
	 ...
	 */
	private boolean estHeure(final char c) {
		return c == 'h';
	}
	 ...
	 * 
	 * @param c
	 *            le caractere a transformer
	 * @return la chaine de caractere qui va bien ^_^
	 */
	private String special(final char c) {
		if (estMonnaie(c)) {
			return monnaie(c);
		}
		switch (c) {
	 ...
	 * @param c
	 *            le caractere a tester
	 * @return true si c est un caractere de monnaie {$, � ou \200} (\200 == euros)
	 */
	private boolean estMonnaie(final char c) {
		return (c == '$') || (c == '�') || (c == '�');
	}
	 ...
	 * 
	 * @param c
	 *            le caractere a prononcer
	 * @return la chaine de caractere correspondant au symbole de la monnaie.
	 */
	private String monnaie(final char c) {
		switch (c) {
			case '$':
				return "_dollar_";
			case '�':
				return "_livre_sterling_";
			default:
				return "";
		}
	}
	 ...
	 */
	private boolean estLettre(final char c) {
	 ...
	 */
	private String abbrev(final String texte, final Indice i) {
		final int l = texte.length();
		String res = "";
		String s = "";
		final int ii = i.val();
		if ((l - ii) >= 2) {
	 ...
	 */
	private int prosodie(final char c) {
		if (estParenthese(c)) {
	 ...
	 */
	private boolean estChiffre(final char c) {
		return (c >= 48) && (c <= 57);
	}
	 ...
	 * 
	 * @param s
	 *            la chaine de caractere a transcoder
	 * @param cs1
	 *            le charset initial
	 * @param cs2
	 *            le charset voulu
	 ...
	 */
	private static String encode(final String s, final String cs1, final String cs2) {
		String res = "";
		try {
			final byte[] b = s.getBytes(cs1);
